...
const SpamDetector = () => {
  ...
  const checkBackendHealth = async () => {
    try {
      ...
      setIsBackendConnected(health.model_loaded);
  ...
  const handleAnalyze = async () => {
    ...
    try {
      // Call the real API
      const apiResult = await apiService.predictEmail(emailText);
      
      // Transform API response to match UI format
      const transformedResult = {
        isSpam: apiResult.isSpam,
        confidence: apiResult.confidence,
        label: apiResult.isSpam ? "SPAM DETECTED" : "SAFE EMAIL",
        message: apiResult.message,
        detectedPatterns: apiResult.detectedPatterns
      };

      setResult(transformedResult);
      setSystemStatus(apiResult.isSpam ? 'spam' : 'safe');
      
    } catch (error) {
      console.error('Analysis error:', error);
      setError(error.message || 'Failed to analyze email. Please try again.');
      ...
      if (error.message.includes('Cannot connect to server')) {
        ...
        setError('Using offline mode - backend not connected');
      }
    } finally {
    ...
    <section id="detector" className="py-24 relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-6">
        ...
        >
          ...
          >
            <motion.div
              className={`w-2 h-2 rounded-full ${isBackendConnected ? 'bg-emerald-400' : 'bg-amber-400'}`}
              animate={{ scale: [1, 1.3, 1], opacity: [1, 0.5, 1] }}
              transition={{ duration: 1.5, repeat: Infinity }}
            />
            <span className="text-sm font-mono uppercase tracking-widest text-cyan-400">
              {isBackendConnected ? 'Live Detection - API Connected' : 'Offline Mode'}
            </span>
          </motion.div>
            ...
            <div className="px-6 py-3 border-t border-gray-800/50 bg-gradient-to-r from-gray-900/50 to-gray-800/20">
              <div className="flex items-center justify-between text-[10px] font-mono text-gray-600">
                <div className="flex items-center gap-4">
                  ...
                  <span>INBOXXORCIST</span>
                </div>
                <div className="flex items-center gap-4">
                  <span>MODEL: SVM-SPAM-V1</span>
                  <span>|</span>
